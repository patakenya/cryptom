...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_deposit'])) {
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            ...
            if ($transaction_id && in_array($action, ['approve', 'reject'])) {
                $stmt = $pdo->prepare("SELECT t.id, t.user_id, t.amount, t.status, u.full_name, u.email 
                ...
                if ($transaction) {
                    if ($transaction['status'] !== 'pending') {
                        $error = 'Deposit is already ' . $transaction['status'] . '.';
                    } else {
                        ...
                        $subject = 'Your Deposit Has Been ' . ucfirst($action) . 'd';
                        $body = 'Hello ' . htmlspecialchars($transaction['full_name']) . ',<br><br>Your deposit of $' . number_format($transaction['amount'], 2) . ' has been ' . $action . 'd by an admin. ' . ($action === 'approve' ? 'The funds are now available in your account.' : 'Please contact support for more information.') . '<br><br>Thank you!';
                        $alt_body = "Hello " . $transaction['full_name'] . ",\n\nYour deposit of $" . number_format($transaction['amount'], 2) . " has been " . $action . "d by an admin. " . ($action === 'approve' ? 'The funds are now available in your account.' : 'Please contact support for more information.') . "\n\nThank you!";
                        if (sendNotificationEmail($transaction['email'], $transaction['full_name'], $subject, $body, $alt_body)) {
                            $success = 'Deposit ' . $action . 'd successfully and notification sent!';
                        } else {
                            $success = 'Deposit ' . $action . 'd, but failed to send notification email.';
                        }
                        $pdo->commit();
                    }
                } else {
    ...
    error_log('Deposit Management Error: ' . $e->getMessage());
...
<head>
    ...
    <title>Manage Deposits - CryptoMiner ERP</title>
        ...
        <section class="mb-8">
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-semibold text-gray-800">Manage Deposits</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if (empty($deposits)): ?>
                                <tr>
                                    <td colspan="6" class="px-4 py-4 text-center text-gray-500">No pending deposits found.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($deposits as $deposit): ?>
                                    <tr>
                                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($deposit['full_name']); ?></td>
                                        <td class="px-4 py-4 whitespace-nowrap text-sm text-green-600">$<?php echo number_format($deposit['amount'], 2); ?></td>
                                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($deposit['method']); ?></td>
                                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo date('M d, Y H:i', strtotime($deposit['created_at'])); ?></td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800"><?php echo ucfirst($deposit['status']); ?></span>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap text-sm">
                                            <form method="POST" class="inline">
                                                ...
                                                <button type="submit" name="update_deposit" class="text-green-600 hover:text-green-800 mr-2">Approve</button>
                                            </form>
                                            <form method="POST" class="inline">
                                                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                                <input type="hidden" name="transaction_id" value="<?php echo $deposit['id']; ?>">
                                                <input type="hidden" name="action" value="reject">
                                                <button type="submit" name="update_deposit" class="text-red-600 hover:text-red-800">Reject</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
